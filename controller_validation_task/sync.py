...
the operator's Ctrl+Q still aborts — upstream's blocking ``wait_for_ttl``
would sit unresponsive until the scanner fired, which is awkward when a
sequence gets cancelled.

Like :mod:`controller_validation_task.markers`, hardware problems degrade
rather than abort: a port that is unset or will not open logs a warning and
falls back to the ``backend="none"`` behaviour above.
"""
...
if TYPE_CHECKING:  # pragma: no cover - typing only
    ...
    from controller_validation_task.settings import SyncSettings
...
def _unsent(value: int, reason: str) -> Sync:
    ...
    usable behavioural data, whereas one that never started yields none.
    """
...
def configure(settings: SyncSettings, *, stream: markers.StreamConfig | None = None) -> Sync:
    ...
    if not settings.port:
        return _keyboard_waiter(
            settings, "sync.backend is 'serial' but sync.port is not set."
        )
    try:
